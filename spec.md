...
**Spec version: 0.3** | Date: 2026-04-25 | Status: Ready for developer handoff
...
**Reading guide for v0.3:** this revision closes out the v0.2 open questions (six decisions, all recorded in Appendix A) and is intended as the final pre-kickoff spec. Items in **blue bold** mark substantive additions or changes since v0.1. *“Decision (v0.2/v0.3):” callouts* flag architectural choices that belong in docs/DECISIONS.md once the plugin repo is created.
...
| Score calculation  | Plugin calculates total score, max score (= count × maxvalue), and matching result band. Percentage is calculated but hidden by default.             |
...
| Minimum scale value         | integer         | 1                              | Validate min \< max. Negatives allowed (e.g., -5..+5 for risk-style scales) but document the implication for percentage.                                                                                                                                                                                                                                                        |
| Maximum scale value         | integer         | 10                             | Recommended practical maximum: 20.                                                                                                                                                                                                                                                                                                                                              |
...
| Show previous attempt       | boolean         | Yes                            | If retakes off and user has an attempt, display existing result.                                                                                                                                                                                                                                                                                                                |
...
On save of the band-management screen, the plugin computes the union of all band ranges and compares to the theoretical score range (count of items × minvalue) to (count of items × maxvalue).
...
11. Create Scorecard activity from the activity chooser.

12. Configure general settings on mod_form.php.

13. After saving, teacher lands on the Manage screen if no items exist (single tabbed page: Items \| Bands \| Reports).

14. Teacher adds prompts and optional low/high anchors. Reorders with up/down arrows.

15. Teacher defines result bands. Save validates coverage (§4.3).

16. Teacher previews learner view (read-only render with no submission action).

17. Teacher reviews submissions from the Reports tab and exports CSV.
...
- Changing minvalue or maxvalue: blocked once any attempt exists, because re-scoring history is ambiguous. Teacher must duplicate the activity instead.
...
- Target Moodle version: 5.1 and above (requires = 2024100100 — verify exact value against the live codebase before commit).

- Use Moodle core APIs for forms, output rendering, capabilities, completion, gradebook, backup/restore, and privacy.

> ***Note:** Per LMS Light convention (CONTEXT.md): every referenced framework API — events, classes, capabilities, config keys — must be grep-verified against the target Moodle source before being relied upon in the spec or implementation. The requires value above is approximate; confirm it during Phase 1.*
...
**Index:** (scorecardid, sortorder) for the manage and learner-view queries.
...
**Index:** (scorecardid, minscore) for band-matching queries.
...
| maxscore                  | int                   | Snapshot: count of visible non-deleted items × maxvalue at submission time. Use this for the result-screen denominator, not a recomputed value. |
| percentage                | decimal(5,2)          | Stored at submission. Display gated by scorecard.showpercentage. Width tightened from v0.1.                                                     |
...
**Indices:** (scorecardid, userid) for the “does this user have an attempt?” lookup, and (scorecardid, timecreated) for the report sort.
...
**Index:** (attemptid) for the result-screen and report detail queries.
...
- When enabled, grade max defaults to visible item count × max scale value, recalculated on item add/remove only while no attempts exist.
...
- Numeric scale rendered as radio buttons from minvalue to maxvalue. WCAG 2.1 AA conformance is the target; consult the Moodle accessibility guidelines.
...
maxscore = count(visibleitems) × scorecard.maxvalue

minpossible = count(visibleitems) × scorecard.minvalue
...
- Validate response values are integers within \[minvalue, maxvalue\].

- Validate band ranges on save: minscore ≤ maxscore, no overlaps with sibling bands.

- Validate scale on save: minvalue \< maxvalue; once attempts exist, block changes to minvalue and maxvalue.

- Prevent duplicate attempts when allowretakes is false (server-side check, not just UI).

- Use Moodle formslib and PARAM\_\* cleaning for all input.
...
| **Phase**                                                | **Deliverables**                                                                                                                                                                                                                                                                                           |
|----------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Phase 1 — Skeleton                                       | Installable mod_scorecard with activity creation, db schema (all five tables incl. snapshot columns and soft-delete flags), capabilities, icon, basic view page. Phase gate: phpcs clean, PHPUnit skeleton tests green, manual install-and-create smoke.                                                   |
| Phase 2 — Authoring                                      | Manage screen with Items and Bands tabs; add/edit/soft-delete/reorder. Band coverage validation. Phase gate: phpcs clean, item and band CRUD tests green, manual authoring smoke including band overlap and gap cases.                                                                                     |
| Phase 3 — Learner Submission                             | Learner form, validation, attempt + response save in transaction, scoring engine, band matching with snapshotting, result display (with showpercentage + showitemsummary toggles). Phase gate: phpcs clean, scoring/band-matching tests green, manual end-to-end smoke with the Career Fit Score fixture.  |
| Phase 4 — Reporting                                      | Teacher Reports tab, expandable detail with deleted-item handling, CSV export with snapshot columns, group mode awareness. Phase gate: phpcs clean, report-data tests green, manual report and export smoke.                                                                                               |
| Phase 5a — Moodle integration (completion + gradebook)   | Activity completion (submission rule), gradebook integration, per-tenant theming hooks (CSS custom properties + styles.css). Phase gate: phpcs clean, completion + gradebook tests green, manual smoke with both off and on.                                                                               |
| Phase 5b — Moodle integration (backup/restore + privacy) | Backup/restore with snapshot fidelity, privacy provider with metadata + export + delete, plus the recommended second test fixture (10-item readiness check) added to QA. Phase gate: phpcs clean, privacy provider tests green, full backup/restore round-trip smoke, real-data privacy export inspection. |
...
| maxscore on the attempt = count × maxvalue (upper-bound metric, not normalized).                                                                                     | If a feature requires a true 0–100 normalized value (add a separate column rather than overloading).                                                                                                                                       |
...
*End of specification — Spec v0.3 — 2026-04-25 — Ready for developer handoff*
